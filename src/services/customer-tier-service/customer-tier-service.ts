...
export function getTierDefaultPlan(tierId: string): string | undefined {
    return customerTiers[tierId]?.defaultPlan;
}

export function getTierStripeProductId(tierId: string): string | undefined {
    return customerTiers[tierId]?.stripeProductId;
}

export function getTierPricingMode(tierId: string): 'FIXED_COST' | 'PER_CONTRIBUTOR' {
    switch (customerTiers[tierId]?.pricingMode) {
        case 'FIXED_COST':
        case undefined:
            return 'FIXED_COST';
        ...
        default:
            throw new Error(`Invalid pricing mode in tier ${tierId}: ${customerTiers[tierId]?.pricingMode}`);
    }
}
...
export function listPaidTiers(): string[] {
    return Object.keys(customerTiers).filter((tierId) => !isFreeTier(tierId) && !isTrialTier(tierId));
}

export function listTiersForAutoDowngrade(): string[] {
    return Object.keys(customerTiers).filter((tierId) => isDowngradableTier(tierId));
}

export function listDowngradablePaidTiers(): string[] {
    return Object.keys(customerTiers).filter((tierId) => isDowngradableTier(tierId) && !isFreeTier(tierId) && !isTrialTier(tierId));
}
