...
import { updateProjectQuantity } from '../services/stripe-service/stripe-service';
...
export interface IProjectDoc extends IProject, Document<Types.ObjectId>, MongooseTimestamps {
    ...
    readonly tier: { isFree: boolean; hooks: customerTierService.TierHooks };
...
export type IProjectJSON = Writeable<Omit<IProject, 'collaborators' | 'collaborationInviteToken' | 'subscription'>> &
    MongooseTimestamps &
    Pick<IProjectDoc, 'id' | 'availableFeatures' | 'tier' | 'subscription' | 'eligibleForTrials' | 'classificationGroups'> & {
...
type ProjectPreviewFields = {
    ...
    wizard: {
        settings: {
            studioVersion: number | undefined;
        };
    };
};
...
export interface IProjectModel extends Model<IProjectDoc>, SoftDeleteModel<IProjectDoc> {
    ...
    updateProject(id: Types.ObjectId | string, update: Partial<IProject>, userId: Types.ObjectId | string): Promise<IProjectDoc | null>;
    ...
    findNonDraftOrganizationProjectIds(organizationId: Types.ObjectId): Promise<Types.ObjectId[]>;
    ...
    cancelSubscription(
        ...
        options?: { immediate?: boolean; skipEmail?: boolean }
    ): Promise<IProjectDoc | null>;
    startSubscription(
        projectId: Types.ObjectId | string,
        subscription: { subscriptionId: string; tierId: string }
    ): Promise<IProjectDoc | null>;
    updateSubscription(projectId: Types.ObjectId | string, update: SubscriptionUpdate): Promise<IProjectDoc | null>;
    ...
    updateAllProjectQuantities(): Promise<void>;
    ...
    findProjectsByRepoId(type: string, id: string): Promise<IProjectDoc[] | null>;
    findProjectsByWebhookId(type: string, id: string): Promise<IProjectDoc[] | null>;
...
ProjectSchema.statics.getPreviewFields = function (project, environmentName?) {
    const response: ProjectPreviewFields = {
        ...
        wizard: {
            settings: {
                studioVersion: project.wizard?.settings?.studioVersion
            }
        }
    };
...
ProjectSchema.statics.updateProject = async function (id, update, userId) {
    ...
    if (updateSet.customDomain) {
        const projectWithDomain = await Project.findOne({
            customDomain: updateSet.customDomain,
            _id: { $ne: id }
        });
        if (projectWithDomain) {
            throw new Error('custom domain is already being used');
        }
    }
...
ProjectSchema.statics.findNonDraftOrganizationProjectIds = async function (organizationId) {
    return (
        await Project.find({ organizationId, buildStatus: { $ne: 'draft' } })
            .select({ _id: 1 })
            .lean()
    ).map((p) => p._id!);
};
...
// This method is used for the PER_CONTRIBUTOR pricing mode.
// INVITED users are excluded just because we don't distinguish at this time between
// invitations to be a Viewer and invitations to be a Contributor. Maybe one day we
// should start charging for unaccepted non-Viewer invites.
ProjectSchema.typeSafeVirtual('subscription', 'contributorCount', function () {
    return (
        1 + // Owner of the project
        (
            this.collaborators?.filter(
                (c) =>
                    ![CollaboratorRole.UNLICENSED.name, CollaboratorRole.VIEWER.name, CollaboratorRole.INVITED.name].includes(c.role ?? '')
            ) ?? []
        ).length
    );
});
...
ProjectSchema.statics.updateAllProjectQuantities = async function () {
    const paidTiers = customerTierService.listPaidTiers();
    const projects = await Project.find({
        'subscription.tierId': { $in: paidTiers }
    });
    for (const project of projects) {
        if (project.subscription.id) {
            await updateProjectQuantity(project);
        }
    }
};
...
ProjectSchema.statics.findProjectsByWebhookId = async function (type, id) {
    switch (type) {
        case 'github':
            return Project.find({ 'webhooks.github.id': id });
        default:
            throw new Error(`${type} isn't supported`);
    }
};

ProjectSchema.statics.findProjectsByRepoId = async function (type, id) {
    switch (type) {
        case 'github':
            return Project.find({ 'deploymentData.github.id': id });
        default:
            throw new Error(`${type} isn't supported`);
    }
};
